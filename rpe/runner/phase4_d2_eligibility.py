...
from rpe.runner.phase4_d2_eligibility_authority import (
    CONFIG_BYTES,
    CONFIG_SHA256,
)
...
REQUIRED_CODE_AUTHORITY_PATHS = (
    ...
    "rpe/perturb/sweep.py", "rpe/runner/d2_selection.py",
    "rpe/runner/phase1_config.py", "rpe/runner/phase1_perturbations.py",
...
def parse_phase4_d2_eligibility_config(
    ...
    *,
    ...
    artifact_payload_files = _strings("artifact_payload_files", config.get("artifact_payload_files", ()))
    code_authority = _object("code_authority", config.get("code_authority", {}))
    environment_authority = _object("environment_authority", config.get("environment_authority", {}))
    trust_anchor = _object("trust_anchor", config.get("trust_anchor", {}))
    if not synthetic_fixture:
        if artifact_payload_files != PAYLOAD_FILES:
            raise Phase4D2EligibilityError("artifact_payload_files", "must be exact payload order through manifest")
        if tuple(code_authority) != REQUIRED_CODE_AUTHORITY_PATHS:
            raise Phase4D2EligibilityError("code_authority", "must have exact frozen authority paths and order")
        for relative_path, receipt_value in code_authority.items():
            ...
            if set(receipt) != {"bytes", "sha256"} or not target.is_file():
                raise Phase4D2EligibilityError(f"code_authority.{relative_path}", "invalid receipt or missing authority path")
            if _int(f"code_authority.{relative_path}.bytes", receipt["bytes"], minimum=1) != target.stat().st_size or str(receipt["sha256"]) != _sha_file(target):
                raise Phase4D2EligibilityError(f"code_authority.{relative_path}", "bytes or SHA-256 mismatch")
        if dict(environment_authority) != REQUIRED_ENVIRONMENT_AUTHORITY:
            raise Phase4D2EligibilityError("environment_authority", "must match frozen runtime environment")
        if dict(trust_anchor) != TRUST_ANCHOR:
            raise Phase4D2EligibilityError("trust_anchor", "must bind production authority module")
        if dict(authorities) != REQUIRED_AUTHORITIES:
            raise Phase4D2EligibilityError("authorities", "must contain exact frozen authority identities")
    elif artifact_payload_files or code_authority or environment_authority or trust_anchor:
        raise Phase4D2EligibilityError("synthetic authority", "synthetic fixtures may only use empty authority mappings")
...
def _reconstruct_real_bacteria_inputs(
    ...
    # A self-consistent artifact digest is insufficient authority: replay the
    # frozen class-local PCG64 selection against the real Bacteria inventory.
    try:
        validate_d2_few_shot_selection(
            selection,
        ...
        raise Phase4D2EligibilityError("selection replay", str(error)) from error
    ...
    for seed_document_value in selection.get("selections", ()):
        seed_document = _object("selection.selections[]", seed_document_value)
        for class_document_value in seed_document.get("classes", ()):
            class_document = _object("selection.classes[]", class_document_value)
            class_label = _int("selection.class_label", class_document.get("class_label"), minimum=0)
            identifiers = [str(value) for value in class_document.get("validation_record_ids", ())]
            for values in _object("selection.train_record_ids", class_document.get("train_record_ids")).values():
        ...
        for batch in loader.iter_batches():
            ...
            for record_id, class_label, stored_intensity, source_row in zip(
                ...
                receipt = {
                    ...
                    "native_axis_sha256": _sha_bytes(native_axis.tobytes(order="C")),
                    "native_intensity_sha256": _sha_bytes(intensity.tobytes(order="C")),
                    "record_id": record_id,
                    "role_count": 0,
    ...
    observed_test_sha = _sha_bytes(
        ("\n".join(test_record_ids) + "\n").encode("utf-8")
    )
    selection_test = _object("selection.test", selection.get("test"))
    if (
        len(test_record_ids) != config.test_record_count
        or observed_test_sha != str(selection_test.get("record_ids_sha256", ""))
    ):
        raise Phase4D2EligibilityError(
            "selection.test",
            "retained test count or record-ID digest mismatch",
        )
    if set(test_class_labels) != set(range(config.class_count)):
        raise Phase4D2EligibilityError(
            "test class labels",
            "must cover the frozen class space",
        )
    ...
    for seed_document_value in selection.get("selections", ()):
        seed_document = _object("selection.selections[]", seed_document_value)
        seed = _int("selection.seed", seed_document.get("seed"), minimum=0)
        train_by_shot: dict[str, list[str]] = {
            str(shot): [] for shot in (5, 10, 20)
        }
        validation_ids: list[str] = []
        for class_document_value in seed_document.get("classes", ()):
            class_document = _object(
                "selection.classes[]", class_document_value
            )
            validation = [
                str(value) for value in class_document.get("validation_record_ids", ())
            ]
            ...
            for shot, identifiers in train_map.items():
                selected = [str(value) for value in identifiers]
                train_by_shot[str(shot)].extend(selected)
                seen_finetune_ids.update(selected)
        validation_tuple = tuple(sorted(validation_ids))
        for shot in (5, 10, 20):
            train_tuple = tuple(sorted(train_by_shot[str(shot)]))
            if set(train_tuple) & set(validation_tuple):
                raise Phase4D2EligibilityError(
                    "selection",
                    "train/validation overlap within one model cell",
                )
            model_cells.append(
                {
                    "seed": seed,
                    "shot_count": shot,
                    "train_record_ids": train_tuple,
                    "validation_record_ids": validation_tuple,
                }
            )
            model_role_occurrences.extend(
                {
                    "record_id": record_id,
                    "role": "train",
                    "seed": seed,
    ...
    if set(finetune_source_records) != seen_finetune_ids:
        raise Phase4D2EligibilityError(
            "finetune source ledger",
            "selected records were not reconstructed exactly",
        )
    role_counts: dict[str, int] = {}
    for row in model_role_occurrences:
    ...
    if (
        len(source_records) != config.source_record_count
        or len(model_cells) != config.model_cell_count
        or len(model_role_occurrences) != config.model_role_occurrence_count
        or len(seen_finetune_ids) != config.finetune_union_record_count
    ):
        raise Phase4D2EligibilityError(
            "real input ledger",
            "frozen denominator mismatch",
        )
    return D2EligibilityInputs(
        ...
        test_record_ids_sha256=observed_test_sha,
...
def reconstruct_d2_eligibility_inputs(
    ...
    if dataset_path.is_dir():
        return _reconstruct_real_bacteria_inputs(
            dataset_path,
            selection_path,
            config,
        )
...
def evaluate_d2_eligibility_gates(
    ...
    *,
    ...
    def grid_state(rows: Sequence[Mapping[str, object]], planned: int) -> Mapping[str, object]:
        ...
        return {
            "planned": planned, "complete": complete, "failed": failed,
    ...
    if has_execution_grids:
        for output_id in METRIC_OUTPUT_IDS[8:]:
            if cwt["state"] != "evaluable":
                metric_outputs[output_id] = {**metric_outputs[output_id], "state": "not_evaluable_incomplete_grid"}
...
def _structure_metric_rows(
    *,
    ...
    """Emit the five structure rows from one canonical metric evaluation."""
    if state != "complete" or candidate_receipt is None:
        return tuple(
            {
                "condition_id": condition_id, "diagnostics_sha256": None,
                "metric_output_id": output_id, "record_id": record_id,
                "result_sha256": None, "state": state,
            }
            for output_id in METRIC_OUTPUT_IDS[8:]
        )
    try:
        ...
        return tuple(
            {
                ...
                "metric_output_id": output_id, "record_id": record_id,
                "result_sha256": None, "state": "failed_runtime",
            }
            for output_id in METRIC_OUTPUT_IDS[8:]
        )
    ...
    return tuple(
        {
            "condition_id": condition_id, "diagnostics_sha256": diagnostics_sha,
            "metric_output_id": output_id, "record_id": record_id,
            "result_sha256": result_sha, "state": "complete",
        }
        for output_id in METRIC_OUTPUT_IDS[8:]
    )


def _scalar_metric_rows(*, record_id: str, condition_id: str, source: Spectrum1D, output: Spectrum1D | None, state: str) -> tuple[Mapping[str, object], ...]:
    if output is None:
        return tuple({"condition_id": condition_id, "diagnostics_sha256": None, "metric_output_id": output_id, "record_id": record_id, "result_sha256": None, "state": state} for output_id in METRIC_OUTPUT_IDS[:8])
    rows = []
    for output_id, metric in _metric_objects().items():
        ...
        try:
            ...
            row = {"condition_id": condition_id, "diagnostics_sha256": _receipt_hash(result.diagnostics), "metric_output_id": output_id, "record_id": record_id, "result_sha256": _receipt_hash(result), "state": "complete"}
        except Exception as error:
            row = {"condition_id": condition_id, "diagnostics_sha256": _receipt_hash({"message": str(error), "type": type(error).__name__}), "metric_output_id": output_id, "record_id": record_id, "result_sha256": None, "state": "failed_runtime"}
        rows.append(row)
    return tuple(rows)


def _execute_d2_record(*, record_order: int, spectrum: Spectrum1D, class_label: int, sweep: PerturbationSweepConfig, phase1_config: Phase1CoreConfig, config: Phase4D2EligibilityConfig, admission: P10MemoryAdmission, cwt_system: Phase3System) -> Mapping[str, object]:
    """Bounded worker: all science objects die before this function returns."""
    record_id = spectrum.spectrum_id.split("::", 1)[1]
    source = _phase1_source_for_d2(spectrum, record_order=record_order, record_id=record_id, class_label=class_label)
    cells: list[Mapping[str, object]] = []; conditions: list[Mapping[str, object]] = []; metrics: list[Mapping[str, object]] = []; cwt_rows: list[Mapping[str, object]] = []
    try:
        reference_receipt = run_peak_detection_system(cwt_system, spectrum)
    except Exception as error:
        reference_receipt = None; reference_state = "failed_runtime"; reference_failure = _receipt_hash({"message": str(error), "type": type(error).__name__})
    else:
        reference_state = "complete"; reference_failure = None
    alpha0_sha = _sha_bytes(project_d2_support(spectrum, config).tobytes(order="C"))
    alpha_zero_failures: list[str] = []
    conditions.append({"class_label": class_label, "condition_id": "alpha0", "output_axis_sha256": _array_sha256(spectrum.axis_cm1), "output_intensity_sha256": _array_sha256(spectrum.intensity), "record_id": record_id, "result_sha256": alpha0_sha, "support_sha256": alpha0_sha, "state": "complete"})
    cwt_rows.append(_cwt_receipt_row(record_id, "alpha0", reference_receipt, reference_state) if reference_receipt is not None else {"condition_id":"alpha0","diagnostics_sha256":reference_failure,"peak_list_sha256":None,"record_id":record_id,"state":reference_state,"warning_sha256":None})
    metrics.extend(_scalar_metric_rows(record_id=record_id, condition_id="alpha0", source=spectrum, output=spectrum, state="complete"))
    metrics.extend(_structure_metric_rows(record_id=record_id, condition_id="alpha0", reference_receipt=reference_receipt, candidate_receipt=reference_receipt, state=reference_state) if reference_receipt is not None else _structure_metric_rows(record_id=record_id, condition_id="alpha0", reference_receipt=None, candidate_receipt=None, state=reference_state))
    for perturbation_id in ALL_PERTURBATION_IDS:
        if perturbation_id in INACTIVE_PERTURBATION_IDS:
            cells.append({"class_label":class_label,"exception":None,"native_gate":{},"outputs":[],"p10_estimated_peak_bytes":None,"perturbation_id":perturbation_id,"reason_code":STRUCTURAL_REASON,"record_id":record_id,"state":"structurally_ineligible","state_digest":None}); continue
        ...
        cells.append({"class_label":class_label,"exception":support_failure if support_failure is not None else _exception_payload(cell),"native_gate":dict(cell.evidence.native_gate),"outputs":[{"alpha_float64_le_hex":r.alpha_float64_le_hex,"diagnostics_sha256":_receipt_hash(r.result.diagnostics),"output_axis_sha256":_array_sha256(r.result.output.axis_cm1),"output_intensity_sha256":_array_sha256(r.result.output.intensity),"output_spectrum_id":r.result.output.spectrum_id,"support_sha256":support_hashes[r.alpha_float64_le_hex]} for r in cell.records] if state == "complete" else [],"p10_estimated_peak_bytes":estimate_p10_peak_bytes(spectrum.axis_cm1.size) if perturbation_id == "p10" else None,"perturbation_id":perturbation_id,"reason_code":cell.reason_code,"record_id":record_id,"state":state,"state_digest":None if cell.state is None else cell.state.state_digest})
        if perturbation_id not in FULL_DOMAIN_PERTURBATION_IDS: continue
        by_alpha = {r.alpha: r for r in cell.records}
        for alpha in POSITIVE_ALPHAS:
            condition_id = f"{perturbation_id}:{np.float64(alpha).tobytes(order='C').hex()}"; perturbed = by_alpha.get(alpha); output = None if perturbed is None or state != "complete" else perturbed.result.output; condition_state = state
            support_sha = None if perturbed is None or state != "complete" else support_hashes.get(perturbed.alpha_float64_le_hex)
            conditions.append({"class_label":class_label,"condition_id":condition_id,"output_axis_sha256":None if output is None else _array_sha256(output.axis_cm1),"output_intensity_sha256":None if output is None else _array_sha256(output.intensity),"record_id":record_id,"result_sha256":None if perturbed is None or output is None else _perturbation_result_hash(perturbed),"support_sha256":support_sha,"state":condition_state})
            try:
                candidate = None if output is None else run_peak_detection_system(cwt_system, output)
            except Exception as error:
                candidate = None; candidate_state = "failed_runtime"; candidate_failure = _receipt_hash({"message":str(error),"type":type(error).__name__})
            else:
                candidate_state = condition_state; candidate_failure = None
            cwt_rows.append(_cwt_receipt_row(record_id, condition_id, candidate, candidate_state) if candidate is not None else {"condition_id":condition_id,"diagnostics_sha256":candidate_failure,"peak_list_sha256":None,"record_id":record_id,"state":candidate_state,"warning_sha256":None})
            metrics.extend(_scalar_metric_rows(record_id=record_id, condition_id=condition_id, source=spectrum, output=output, state=condition_state))
            metrics.extend(_structure_metric_rows(record_id=record_id, condition_id=condition_id, reference_receipt=reference_receipt, candidate_receipt=candidate, state=candidate_state) if reference_receipt is not None else _structure_metric_rows(record_id=record_id, condition_id=condition_id, reference_receipt=None, candidate_receipt=None, state="failed_runtime"))
    if alpha_zero_failures:
        ...
        metrics = [
            ...
            if row["condition_id"] == "alpha0" else row
            for row in metrics
        ]
    return {"record_order":record_order,"cells":tuple(cells),"record_conditions":tuple(conditions),"metric_statuses":tuple(metrics),"cwt_receipts":tuple(cwt_rows),"class_label":class_label,"record_id":record_id,"_worker_pid":os.getpid()}
...
def build_phase4_d2_eligibility_from_inputs(
    ...
    *,
    ...
    if output_path.exists():
        raise Phase4D2EligibilityError(str(output_path), "append-only output target already exists")
    ...
    # `catalog` is intentionally not an input to this focused builder: resolve the frozen CWT authority.
    catalog = load_classical_catalog(ROOT / CATALOG_RELATIVE_PATH)
    ...
    for record in records:
        for key, destination in (("cells", cells), ("record_conditions", record_conditions), ("metric_statuses", metric_statuses), ("cwt_receipts", cwt_receipts)):
            for row in record[key]:
                validate_outcome_blind_d2_payload(row)
                destination.append(row)
    gate = evaluate_d2_eligibility_gates(
        cells, inputs.test_class_labels, config, record_conditions=record_conditions,
        metric_statuses=metric_statuses, cwt_receipts=cwt_receipts,
    )
...
def verify_phase4_d2_eligibility(
    ...
    *,
    ...
    from rpe.runner.phase4_d2_eligibility_verifier import (
        verify_phase4_d2_eligibility as _verify_phase4_d2_eligibility,
    )

    return _verify_phase4_d2_eligibility(path, worker_count=worker_count)
