...
from rpe.runner.phase4_d2_protocol_b_eligibility_authority import (
    CONFIG_BYTES,
    CONFIG_SHA256,
)
...
class Phase4D2ProtocolBEligibilityError(ValueError):
...
def _json_ready(value: object) -> object:
    ...
    if isinstance(value, float) and math.isfinite(value):
        return value
    raise Phase4D2ProtocolBEligibilityError(
        "json",
...
def _object(path: str, value: object) -> Mapping[str, object]:
    if not isinstance(value, Mapping):
        raise Phase4D2ProtocolBEligibilityError(path, "must be an object")
    return value


def _strings(path: str, value: object) -> tuple[str, ...]:
    if not isinstance(value, (list, tuple)):
        raise Phase4D2ProtocolBEligibilityError(path, "must be an array")
    converted = tuple(str(item) for item in value)
    if any(not item for item in converted):
        raise Phase4D2ProtocolBEligibilityError(path, "must contain nonempty strings")
    return converted
...
def _floats(path: str, value: object) -> tuple[float, ...]:
    ...
    if any(not math.isfinite(item) for item in converted):
        raise Phase4D2ProtocolBEligibilityError(path, "must contain finite numbers")
    return converted


def _int(path: str, value: object, *, minimum: int = 0) -> int:
    if isinstance(value, bool) or not isinstance(value, int):
        raise Phase4D2ProtocolBEligibilityError(path, "must be an integer")
    if value < minimum:
        raise Phase4D2ProtocolBEligibilityError(path, "is outside the allowed range")
    return value


def _number(path: str, value: object) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise Phase4D2ProtocolBEligibilityError(path, "must be a number")
    number = float(value)
    if not math.isfinite(number):
        raise Phase4D2ProtocolBEligibilityError(path, "must be finite")
    return number
...
def parse_phase4_d2_protocol_b_eligibility_config(
    ...
    *,
    ...
    try:
        ...
        raise Phase4D2ProtocolBEligibilityError("config", str(error)) from error
    document = _object("config", document_value)
    if raw != _canonical_json_bytes(document):
        raise Phase4D2ProtocolBEligibilityError("config", "must use canonical JSON")
    ...
    if document.get("schema_version") != SCHEMA_VERSION:
        raise Phase4D2ProtocolBEligibilityError("schema_version", "mismatch")
    if document.get("experiment_id") != EXPERIMENT_ID:
        raise Phase4D2ProtocolBEligibilityError("experiment_id", "mismatch")
    if document.get("protocol") != PROTOCOL:
        raise Phase4D2ProtocolBEligibilityError("protocol", "must equal 'B'")
    ...
    if alpha_grid != ALPHA_GRID:
        raise Phase4D2ProtocolBEligibilityError("alpha_grid", "frozen grid mismatch")
    ...
    shot_source_union_counts = dict(
        _object("denominators.shot_source_union_counts", denominators.get("shot_source_union_counts"))
    )
    shot_role_occurrence_counts = dict(
        _object("denominators.shot_role_occurrence_counts", denominators.get("shot_role_occurrence_counts"))
    )
    ...
    shot_counts = tuple(_int("shot_counts[]", value, minimum=1) for value in document.get("shot_counts", ()))
    if shot_counts != (5, 10, 20):
        raise Phase4D2ProtocolBEligibilityError("shot_counts", "must equal [5,10,20]")
    ...
    if len(support_coordinates_cm1) != support_point_count:
        raise Phase4D2ProtocolBEligibilityError("support_grid", "point count mismatch")
    ...
    if (
        ...
        raise Phase4D2ProtocolBEligibilityError("p10", "frozen contract mismatch")
    ...
    authorities = MappingProxyType(
        {str(key): value for key, value in _object("authorities", document.get("authorities", {})).items()}
    )
    frozen_identities = MappingProxyType(
        {
            str(key): _freeze(value)
            for key, value in _object("frozen_identities", document.get("frozen_identities")).items()
        }
    )
    inherited_rulings = MappingProxyType(
        {
            str(key): _freeze(value)
    ...
    code_authority = MappingProxyType(
        {
            str(key): MappingProxyType(
                {
                    "bytes": _int(f"code_authority.{key}.bytes", value["bytes"], minimum=1),
    ...
    if not claim_boundary:
        raise Phase4D2ProtocolBEligibilityError("claim_boundary", "must be nonempty")
    ...
    if synthetic_fixture:
        ...
        if trust_anchor.get("config_authority_relative_path") != CONFIG_AUTHORITY_RELATIVE_PATH:
            raise Phase4D2ProtocolBEligibilityError("trust_anchor", "authority path mismatch")
    ...
    return Phase4D2ProtocolBEligibilityConfig(
        ...
        shot_source_union_counts=MappingProxyType(
            {str(key): _int(f"shot_source_union_counts.{key}", value, minimum=1) for key, value in shot_source_union_counts.items()}
        ),
        shot_role_occurrence_counts=MappingProxyType(
            {str(key): _int(f"shot_role_occurrence_counts.{key}", value, minimum=1) for key, value in shot_role_occurrence_counts.items()}
        ),
...
def _load_json_document(path: Path) -> Mapping[str, object]:
    try:
        ...
        raise Phase4D2ProtocolBEligibilityError(str(path), str(error)) from error
    if raw != _canonical_json_bytes(value):
        raise Phase4D2ProtocolBEligibilityError(str(path), "must be canonical JSON")
    return _object(str(path), value)


def _reverse_native_record(record: Mapping[str, object], stored_axis: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
    intensity_descending = np.asarray(record["stored_intensity"], dtype="<f4")
    if intensity_descending.ndim != 1 or intensity_descending.size != stored_axis.size:
        raise Phase4D2ProtocolBEligibilityError(
            "records.stored_intensity",
            "must align with the shared native axis",
        )
    axis = stored_axis[::-1].astype("<f8")
    intensity = intensity_descending[::-1].astype("<f8")
...
def project_d2_protocol_b_support(
    ...
    if axis.ndim != 1 or intensity.ndim != 1 or axis.size != intensity.size:
        raise Phase4D2ProtocolBEligibilityError("spectrum", "axis/intensity mismatch")
    if float(axis[0]) > float(support[0]) or float(axis[-1]) < float(support[-1]):
        raise Phase4D2ProtocolBEligibilityError("support_projection", "extrapolation required")
    ...
    if output.size != config.support_point_count:
        raise Phase4D2ProtocolBEligibilityError("support_projection", "point count mismatch")
    if not np.isfinite(output).all():
        raise Phase4D2ProtocolBEligibilityError("support_projection", "must be finite")
    if float(np.linalg.norm(output.astype(np.float64))) <= 0.0:
        raise Phase4D2ProtocolBEligibilityError("support_projection", "zero norm")
    return output


def _reconstruct_synthetic_inputs(
    dataset_path: Path,
    selection_path: Path,
    config: Phase4D2ProtocolBEligibilityConfig,
    ...
    stored_axis = np.asarray(dataset["native_axis_cm1_decreasing_f32"], dtype="<f4")
    if not np.all(np.diff(stored_axis) < 0.0):
        raise Phase4D2ProtocolBEligibilityError(
            "native_axis_cm1_decreasing_f32",
            "must be strictly decreasing",
        )
    source_records = []
    source_spectra = []
    record_id_to_index = {}
    for record_order, record in enumerate(dataset["records"]):
        record_id = str(record["record_id"])
        axis, intensity = _reverse_native_record(record, stored_axis)
        class_label = _int("record.class_label", record["class_label"], minimum=0)
        spectrum = Spectrum1D(
        ...
        record_id_to_index[record_id] = record_order
        source_records.append(
            {
                "class_label": class_label,
                "native_axis_sha256": _array_sha(axis, dtype="<f8"),
                "native_intensity_sha256": _array_sha(intensity, dtype="<f8"),
                "record_id": record_id,
                "record_order": record_order,
                "scope": str(record["source_split"]),
                "source_row": _int("record.source_row", record["source_row"], minimum=0),
    ...
    for seed_document in selection["selections"]:
        ...
        validation_by_class: dict[int, list[str]] = {}
        train_by_class: dict[int, dict[int, list[str]]] = {}
        for class_document in seed_document["classes"]:
            class_label = _int("selection.class_label", class_document["class_label"], minimum=0)
            validation_ids = [str(value) for value in class_document["validation_record_ids"]]
            validation_by_class[class_label] = validation_ids
            shot_map = _object("selection.train_record_ids", class_document["train_record_ids"])
            train_by_class[class_label] = {
                int(shot): [str(value) for value in values] for shot, values in shot_map.items()
            }
            selected_record_ids.update(validation_ids)
            for identifiers in train_by_class[class_label].values():
                selected_record_ids.update(identifiers)
        all_test_ids = [
            str(row["record_id"])
        ...
        for shot_count in config.shot_counts:
            train_ids = sorted(
                value
                for class_values in train_by_class.values()
            ...
            if set(train_ids) & set(validation_ids):
                raise Phase4D2ProtocolBEligibilityError("selection", "train/validation overlap")
            model_cells.append(
                {
                    "seed": seed,
                    ...
                    "test_record_ids": tuple(all_test_ids),
                }
            )
    ...
    shot_memberships: dict[str, set[int]] = {record_id: set() for record_id in record_id_to_index}
    for cell in model_cells:
        shot_count = int(cell["shot_count"])
        for role_name in ("train_record_ids", "validation_record_ids", "test_record_ids"):
            for record_id in cell[role_name]:
                shot_memberships[str(record_id)].add(shot_count)
        ...
        source_rows.append(
            {
                **row,
                ...
                "shot_memberships": sorted(shot_memberships[record_id]),
            }
        )
    if len(source_rows) != config.source_record_count:
        raise Phase4D2ProtocolBEligibilityError("source_records", "count mismatch")
    if len(model_cells) != config.model_cell_count:
        raise Phase4D2ProtocolBEligibilityError("model_cells", "count mismatch")
    if len(model_role_occurrences) != config.model_role_occurrence_count:
        raise Phase4D2ProtocolBEligibilityError("model_role_occurrences", "count mismatch")
    return D2ProtocolBEligibilityInputs(
        source_records=tuple(source_rows),
        model_cells=tuple(
            sorted(model_cells, key=lambda row: (int(row["seed"]), int(row["shot_count"])))
        ),
        model_role_occurrences=tuple(
            sorted(
                ...
                key=lambda row: (
                    int(row["seed"]),
                    int(row["shot_count"]),
                    0 if str(row["role"]) == "train" else 1 if str(row["role"]) == "validation" else 2,
                    record_id_to_index[str(row["record_id"])],
                ),
            )
        ),
        source_spectra=tuple(
            source_spectra[row["record_order"]] for row in source_rows
        ),
        source_record_ids_sha256=_ids_digest([str(row["record_id"]) for row in source_rows]),
        native_axis_point_count=int(source_spectra[0].axis_cm1.size),
...
def _reconstruct_real_inputs(
    ...
    try:
        ...
        raise Phase4D2ProtocolBEligibilityError("selection replay", str(error)) from error
    ...
    for seed_document in selection["selections"]:
        ...
        for class_document in seed_document["classes"]:
            ...
            train_map = _object("selection.train_record_ids", class_document["train_record_ids"])
            per_class_train[class_label] = {
                int(shot): [str(value) for value in values] for shot, values in train_map.items()
            }
            ...
            for record_id in validation_ids:
                source_shot_memberships.setdefault(record_id, set()).update(config.shot_counts)
        for shot_count in config.shot_counts:
            train_ids = sorted(
                value
            ...
            validation_ids = sorted(
                value
                for values in per_class_validation.values()
                for value in values
            )
            if set(train_ids) & set(validation_ids):
                raise Phase4D2ProtocolBEligibilityError("selection", "train/validation overlap")
    ...
    with BacteriaIdBatchLoader(dataset_path, batch_size=4096) as loader:
        for batch in loader.iter_batches():
            ...
            if not np.all(np.diff(stored_axis) < 0.0):
                raise Phase4D2ProtocolBEligibilityError("BacteriaIdBatchLoader", "stored axis must be strictly decreasing")
    ...
    record_order = {str(row["record_id"]): int(row["record_order"]) for row in source_records}
    model_cells = []
    model_role_occurrences = []
    ...
    for cell in cell_specs:
        ...
        model_cell = {
            **cell,
            "test_record_ids": tuple(test_record_ids),
        }
    ...
    return D2ProtocolBEligibilityInputs(
        ...
        model_cells=tuple(sorted(model_cells, key=lambda row: (int(row["seed"]), int(row["shot_count"])))),
        model_role_occurrences=tuple(
            sorted(
                model_role_occurrences,
                key=lambda row: (
        ...
        source_record_ids_sha256=_ids_digest([str(row["record_id"]) for row in final_source_records]),
...
def _validate_real_reconstruction_identities(
    ...
    *,
    ...
    expected_model_seeds = tuple(
        sorted(_int("frozen_identities.model_seed_ids[]", value, minimum=0) for value in frozen.get("model_seed_ids", ()))
    )
...
def reconstruct_d2_protocol_b_inputs(
    dataset_path: Path,
    selection_path: Path,
    config: Phase4D2ProtocolBEligibilityConfig,
) -> D2ProtocolBEligibilityInputs:
    ...
    if dataset_path.is_dir():
        return _reconstruct_real_inputs(dataset_path, selection_path, config)
    return _reconstruct_synthetic_inputs(dataset_path, selection_path, config)
    ...
    *,
    ...
    if cell.status is CellStatus.COMPLETE:
        ...
        for perturbed in cell.records:
            ...
            support_hash, support_point_count, support_max_gap = _project_support_receipt(output, config)
            outputs.append(
                {
...
def _execute_initialized_record(record: Mapping[str, object], spectrum: Spectrum1D) -> tuple[dict[str, object], ...]:
    if _PROCESS_SWEEP is None or _PROCESS_PHASE1_CONFIG is None or _PROCESS_CONFIG is None:
        raise Phase4D2ProtocolBEligibilityError("worker", "process not initialized")
    ...
    for perturbation_id in ACTIVE_PERTURBATION_IDS:
        ...
        try:
            ...
            if perturbation_id == "p10" and p10_estimate is not None and p10_estimate > _PROCESS_CONFIG.p10_memory_budget_bytes:
                raise Phase4D2ProtocolBEligibilityError(
                    "p10 admission",
...
def _bounded_process_count(
    ...
    if requested_workers < 1:
        raise Phase4D2ProtocolBEligibilityError("worker_count", "must be positive")
    if not point_counts:
        return 1
...
def _run_record_jobs(
    ...
    process_count = _bounded_process_count(
        worker_count,
        point_counts,
    ...
    if process_count == 1:
        ...
        results = tuple(
            ...
            for record, spectrum in zip(inputs.source_records, inputs.source_spectra, strict=True)
        )
        _PROCESS_SWEEP = None
    ...
    with ProcessPoolExecutor(
        ...
        futures = [
            executor.submit(_execute_initialized_record, record, spectrum)
            for record, spectrum in zip(inputs.source_records, inputs.source_spectra, strict=True)
        ]
...
def _build_record_conditions(
    ...
    source_by_id = {str(row["record_id"]): row for row in inputs.source_records}
    spectrum_by_id = {
    ...
    for source_record in inputs.source_records:
        ...
        support_hash, support_point_count, max_gap = _project_support_receipt(spectrum, config)
        conditions.append(
            {
                "alpha": 0.0,
        ...
        for perturbation_id in ACTIVE_PERTURBATION_IDS:
            ...
            if str(cell["state"]) == "complete":
                ...
                alpha0 = outputs[0]
                if (
                    str(alpha0["output_axis_sha256"]) != str(source_record["native_axis_sha256"])
                    or str(alpha0["output_intensity_sha256"]) != str(source_record["native_intensity_sha256"])
                ):
                    raise Phase4D2ProtocolBEligibilityError(
                        "alpha-zero collapse",
                    ...
                    conditions.append(
                        {
                            ...
                            "record_order": int(source_record["record_order"]),
                            "state": "complete",
                            "support_max_in_range_gap_cm1": float(output["support_max_in_range_gap_cm1"]),
...
def evaluate_d2_protocol_b_all_role_gates(
    ...
    if len(source_records) != config.source_record_count:
        raise Phase4D2ProtocolBEligibilityError("source_records", "denominator mismatch")
    if len(model_cells) != config.model_cell_count:
        raise Phase4D2ProtocolBEligibilityError("model_cells", "denominator mismatch")
    if len(model_role_occurrences) != config.model_role_occurrence_count:
        raise Phase4D2ProtocolBEligibilityError("model_role_occurrences", "denominator mismatch")
    if len(operator_cells) != config.expected_operator_cell_count:
        raise Phase4D2ProtocolBEligibilityError("operator_cells", "count mismatch")
    ...
    if set(cell_by_key) != expected_keys:
        raise Phase4D2ProtocolBEligibilityError("operator_cells", "grid keys mismatch")
    class_rows = []
    ...
    for shot_count in config.shot_counts:
        ...
        for perturbation_id in ACTIVE_PERTURBATION_IDS:
            states = Counter(
                str(cell_by_key[(record_id, perturbation_id)]["state"]) for record_id in shot_records
            )
            complete_records = {
                record_id
                for record_id in shot_records
            ...
            for class_label in sorted(class_to_records):
                ...
                complete_record_count = sum(record_id in complete_records for record_id in record_ids)
...
def build_phase4_d2_protocol_b_eligibility_from_inputs(
    output_dir: Path,
    *,
    inputs: D2ProtocolBEligibilityInputs,
    sweep: object,
    ...
    if not isinstance(sweep, PerturbationSweepConfig):
        raise Phase4D2ProtocolBEligibilityError("sweep", "must be PerturbationSweepConfig")
    if not isinstance(phase1_config, Phase1CoreConfig):
        raise Phase4D2ProtocolBEligibilityError("phase1_config", "must be Phase1CoreConfig")
    if isinstance(worker_count, bool) or not isinstance(worker_count, int) or worker_count < 1:
        raise Phase4D2ProtocolBEligibilityError("worker_count", "must be a positive integer")
    output_path = Path(output_dir)
    if output_path.exists():
        raise Phase4D2ProtocolBEligibilityError(str(output_path), "append-only output target already exists")
    code = (
    ...
    environment = (
        dict(config.environment_authority) if config.synthetic_fixture else _environment_document()
    )
    point_counts = tuple(spectrum.axis_cm1.size for spectrum in inputs.source_spectra)
    _bounded_process_count(worker_count, point_counts, config.p10_memory_budget_bytes)
    ...
    if len(record_conditions) != config.expected_canonical_record_condition_count:
        raise Phase4D2ProtocolBEligibilityError("record_conditions", "count mismatch")
    ...
    if len(class_summaries) != config.expected_class_summary_count:
        raise Phase4D2ProtocolBEligibilityError("class_summaries", "count mismatch")
    ...
    for name in ordered_files:
        (output_path / name).write_bytes(payloads[name])
    checksum = "".join(
        f"{_sha_bytes(payloads[name])}  {name}\n" for name in ordered_files
    ).encode("utf-8")
    (output_path / "SHA256SUMS").write_bytes(checksum)
...
def build_phase4_d2_protocol_b_eligibility(
    output_root: Path,
    *,
    worker_count: int = 16,
) -> Phase4D2ProtocolBEligibilitySummary:
    config = load_phase4_d2_protocol_b_eligibility_config(ROOT / CONFIG_RELATIVE_PATH)
    ...
    inputs = reconstruct_d2_protocol_b_inputs(
        ROOT / DATASET_RELATIVE_PATH,
        ROOT / SELECTION_RELATIVE_PATH,
        config,
    )
    run_id, _ = _run_identity(config, _code_document(), _environment_document())
    return build_phase4_d2_protocol_b_eligibility_from_inputs(
