...
class Phase4D4EligibilityError(ValueError):
...
def _json_ready(value: object) -> object:
    ...
    if isinstance(value, float) and math.isfinite(value):
        return value
    raise Phase4D4EligibilityError(
        "json", f"unsupported value {type(value).__name__}"
    )


def _reject_nonfinite(value: str) -> None:
    raise Phase4D4EligibilityError("config", f"unsupported JSON constant {value!r}")
...
def _load_json_document(path: Path) -> Mapping[str, object]:
    ...
    except Phase4D4EligibilityError:
        raise
    except (OSError, UnicodeDecodeError, json.JSONDecodeError) as error:
        raise Phase4D4EligibilityError(str(path), str(error)) from error
    if raw != _canonical_json_bytes(document):
        raise Phase4D4EligibilityError(str(path), "must use canonical JSON")
    if not isinstance(document, Mapping):
        raise Phase4D4EligibilityError(str(path), "must be a JSON object")
    return document


def _object(path: str, value: object) -> Mapping[str, object]:
    if not isinstance(value, Mapping):
        raise Phase4D4EligibilityError(path, "must be an object")
    return value


def _float(path: str, value: object) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise Phase4D4EligibilityError(path, "must be a number")
    result = float(value)
    if not math.isfinite(result):
        raise Phase4D4EligibilityError(path, "must be finite")
    return result


def _int(path: str, value: object, *, minimum: int | None = None) -> int:
    if isinstance(value, bool) or not isinstance(value, int):
        raise Phase4D4EligibilityError(path, "must be an integer")
    if minimum is not None and value < minimum:
        raise Phase4D4EligibilityError(path, f"must be >= {minimum}")
    return value


def _string_tuple(path: str, value: object) -> tuple[str, ...]:
    if not isinstance(value, list) or any(not isinstance(item, str) for item in value):
        raise Phase4D4EligibilityError(path, "must be an array of strings")
    return tuple(value)


def _float_tuple(path: str, value: object) -> tuple[float, ...]:
    if not isinstance(value, list):
        raise Phase4D4EligibilityError(path, "must be an array")
...
def _validate_real_authorities(
    *,
    ...
    expected_authorities = _authority_document()
    if dict(authorities) != expected_authorities:
        raise Phase4D4EligibilityError("authorities", "must contain exact frozen receipts")
    if tuple(code_authority) != CODE_AUTHORITY_PATHS:
        raise Phase4D4EligibilityError("code_authority", "path set or order mismatch")
    for relative_path, receipt_value in code_authority.items():
        ...
        if (
            ...
            raise Phase4D4EligibilityError("code_authority", "invalid receipt")
    if dict(environment_authority) != _environment_authority_document():
        raise Phase4D4EligibilityError("environment_authority", "mismatch")
    if dict(trust_anchor) != dict(TRUST_ANCHOR):
        raise Phase4D4EligibilityError("trust_anchor", "mismatch")
    if not verify_live_files:
        return
    if dict(code_authority) != _code_authority_document():
        raise Phase4D4EligibilityError("code_authority", "bytes or SHA-256 mismatch")
    ...
    with zipfile.ZipFile(ROOT / ARCHIVE_RELATIVE_PATH) as archive:
        for key, (member_path, expected_bytes, expected_sha256) in AUTHORITY_MEMBER_RECEIPTS.items():
            try:
                ...
                raise Phase4D4EligibilityError(f"authorities.{key}", str(error)) from error
...
def _support_axis(config: Phase4D4EligibilityConfig) -> np.ndarray:
    ...
    if values.size != config.support_point_count:
        raise Phase4D4EligibilityError("support_grid", "point count mismatch")
    if config.support_axis_float64_sha256 and _array_sha(values, dtype="<f8") != config.support_axis_float64_sha256:
        raise Phase4D4EligibilityError("support_grid", "support axis float64 hash mismatch")
    return values
...
def _validate_native_axis(native_axis: np.ndarray, config: Phase4D4EligibilityConfig) -> None:
    ...
    if axis_f8.ndim != 1 or axis_f8.size < config.support_point_count:
        raise Phase4D4EligibilityError("support_projection", "native support too short")
    if not np.isfinite(axis_f8).all():
        raise Phase4D4EligibilityError("support_projection", "native axis must be finite")
    if not np.all(np.diff(axis_f8) > 0.0):
        raise Phase4D4EligibilityError("support_projection", "native axis must be strictly increasing")
    if config.native_axis_float32_sha256 and _array_sha(axis_f4, dtype="<f4") != config.native_axis_float32_sha256:
        raise Phase4D4EligibilityError("support_projection", "native axis float32 hash mismatch")
    ...
    if native_axis_f64_sha256 and _array_sha(axis_f8, dtype="<f8") != native_axis_f64_sha256:
        raise Phase4D4EligibilityError("support_projection", "native axis float64 hash mismatch")
    support = _support_axis(config)
    if support.size != config.support_point_count:
        raise Phase4D4EligibilityError("support_projection", "point count mismatch")
    if not np.isclose(float(support[0]), config.support_first_cm1, rtol=0.0, atol=1e-12):
        raise Phase4D4EligibilityError("support_projection", "support first point mismatch")
    if not np.isclose(float(support[-1]), config.support_last_cm1, rtol=0.0, atol=1e-12):
        raise Phase4D4EligibilityError("support_projection", "support last point mismatch")
    if config.support_axis_float64_sha256 and _array_sha(support, dtype="<f8") != config.support_axis_float64_sha256:
        raise Phase4D4EligibilityError("support_projection", "support axis float64 hash mismatch")
    ...
    if support_axis_f32_sha256 and _array_sha(support, dtype="<f4") != support_axis_f32_sha256:
        raise Phase4D4EligibilityError("support_projection", "support axis float32 hash mismatch")
    if float(np.max(np.diff(support))) > config.support_max_gap_cm1:
        raise Phase4D4EligibilityError("support_projection", "support gap exceeds the frozen gate")
...
def parse_phase4_d4_eligibility_config(
    ...
    *,
    ...
    if require_frozen_identity and (len(raw_bytes) != CONFIG_BYTES or sha256 != CONFIG_SHA256):
        raise Phase4D4EligibilityError(str(path), "frozen config identity mismatch")
    ...
    if schema_version != SCHEMA_VERSION or experiment_id != EXPERIMENT_ID:
        raise Phase4D4EligibilityError("config", "schema or experiment identity mismatch")
    ...
    if active != ACTIVE_PERTURBATION_IDS:
        raise Phase4D4EligibilityError("active_perturbation_ids", "must equal the frozen perturbation set")
    if inactive != INACTIVE_PERTURBATION_IDS:
        raise Phase4D4EligibilityError("inactive_perturbation_ids", "must equal the frozen perturbation set")
    if full_domain != FULL_DOMAIN_PERTURBATION_IDS:
        raise Phase4D4EligibilityError("full_domain_perturbation_ids", "must equal the frozen perturbation set")
    if peak != PEAK_PERTURBATION_IDS:
        raise Phase4D4EligibilityError("peak_perturbation_ids", "must equal the frozen perturbation set")
    if alpha_grid != ALPHA_GRID:
        raise Phase4D4EligibilityError("alpha_grid", "must equal the frozen alpha grid")
    if metric_ids != METRIC_OUTPUT_IDS:
        raise Phase4D4EligibilityError("metric_output_ids", "must equal the frozen metric output set")
    ...
    if authorities_value is None:
        if synthetic_fixture:
            ...
            raise Phase4D4EligibilityError("authorities", "must be an object")
    else:
    ...
    if coordinates is not None:
        ...
        if len(support_coordinates) == 0:
            raise Phase4D4EligibilityError("support_grid.coordinates_cm1", "must not be empty")
...
def reconstruct_d4_eligibility_inputs(
    ...
    if len(cohort.record_ids) != config.mixture_record_count:
        raise Phase4D4EligibilityError("cohort.record_ids", "mixture record count mismatch")
    if len(cohort.blank_record_ids) != config.blank_record_count:
        raise Phase4D4EligibilityError("cohort.blank_record_ids", "blank record count mismatch")
    ...
    if len(mixture_wells) != config.mixture_well_count:
        raise Phase4D4EligibilityError("cohort.well_ids", "mixture well count mismatch")
    if len(blank_wells) != config.blank_well_count:
        raise Phase4D4EligibilityError("cohort.blank_well_ids", "blank well count mismatch")
    ...
    if len(model_cells) != config.model_cell_count:
        raise Phase4D4EligibilityError("model_cells", "count mismatch")
    if len(model_role_occurrences) != config.model_role_occurrence_count:
        raise Phase4D4EligibilityError("model_role_occurrences", "count mismatch")
...
def project_d4_support(spectrum: Spectrum1D, config: Phase4D4EligibilityConfig) -> np.ndarray:
    ...
    if axis.ndim != 1 or intensity.ndim != 1 or axis.size != intensity.size:
        raise Phase4D4EligibilityError("spectrum", "axis and intensity must be aligned one-dimensional arrays")
    support = _support_axis(config)
    ...
    if not np.isfinite(output).all():
        raise Phase4D4EligibilityError("support_projection", "must be finite")
    return output
...
def _project_to_support_axis(
    *,
    ...
    if float(axis[0]) > float(support[0]) or float(axis[-1]) < float(support[-1]):
        raise Phase4D4EligibilityError("support_projection", "extrapolation required")
    right = int(np.searchsorted(axis, support[-1], side="right"))
    native_in_range = axis[left:right]
    if native_in_range.size < 2:
        raise Phase4D4EligibilityError("support_projection", "insufficient in-range native support")
        ...
        if isinstance(current, Mapping):
            for key, item in current.items():
                ...
                if name in FORBIDDEN_PAYLOAD_KEYS or (
                    ...
                    raise Phase4D4EligibilityError(path, name)
        ...
        if perturbation_id in FULL_DOMAIN_PERTURBATION_IDS:
            ...
            for alpha in POSITIVE_ALPHAS:
                ...
                if state == "complete" and perturbed is not None:
                    ...
                    projected = project_d4_support(native_output, config)
...
def _bounded_process_count(*, requested_workers: int, point_counts: Sequence[int], memory_budget_bytes: int) -> int:
    ...
    if capacity < 1:
        raise Phase4D4EligibilityError("p10.memory_budget_bytes", "cannot admit the largest source record")
...
def _initialize_d4_process(sweep_path: str, phase1_config_path: str, config_path: str, config_raw: bytes) -> None:
    ...
    if len(matches) != 1:
        raise Phase4D4EligibilityError("CWT system", "must resolve exactly once")
...
def _execute_initialized_d4_record(record_order: int, spectrum: Spectrum1D) -> Mapping[str, object]:
    if _PROCESS_SWEEP is None or _PROCESS_PHASE1_CONFIG is None or _PROCESS_D4_CONFIG is None or _PROCESS_CWT_SYSTEM is None:
        raise Phase4D4EligibilityError("process worker", "was not initialized")
...
def build_phase4_d4_eligibility_from_inputs(
    ...
    *,
    ...
    if isinstance(worker_count, bool) or not isinstance(worker_count, int) or worker_count < 1:
        raise Phase4D4EligibilityError("worker_count", "must be a positive integer")
    output_path = Path(output_dir)
    if output_path.exists():
        raise Phase4D4EligibilityError(str(output_path), "append-only output target already exists")
    ...
    for spectrum in inputs.blank_spectra:
        ...
        for perturbation_id in FULL_DOMAIN_PERTURBATION_IDS:
            ...
            for alpha in POSITIVE_ALPHAS:
                cwt_row = None
                if state == "complete" and float(alpha) in records_by_alpha:
                    projected = project_d4_support(records_by_alpha[float(alpha)].result.output, config)
                    native_cwt = _cwt_receipt_from_spectrum(cwt_system, records_by_alpha[float(alpha)].result.output)
                    cwt_row = {
...
def build_phase4_d4_eligibility(
    output_root: Path,
    *,
    worker_count: int = 16,
) -> Phase4D4EligibilitySummary:
    ...
    run_id = _build_run_id(config, inputs)
    return build_phase4_d4_eligibility_from_inputs(
...
def _compare_tree(candidate_path: Path, rebuilt_path: Path) -> None:
    ...
    if candidate_files != rebuilt_files:
        raise Phase4D4EligibilityError("artifact inventory", "candidate and rebuilt file sets differ")
    for name in candidate_files:
        if (candidate_path / name).read_bytes() != (rebuilt_path / name).read_bytes():
            raise Phase4D4EligibilityError(name, "candidate and rebuilt bytes differ")


def verify_phase4_d4_eligibility(path: Path, *, worker_count: int = 12) -> Phase4D4EligibilitySummary:
    from rpe.runner.phase4_d4_eligibility_verifier import verify_phase4_d4_eligibility as _verify_phase4_d4_eligibility

    return _verify_phase4_d4_eligibility(path, worker_count=worker_count)
