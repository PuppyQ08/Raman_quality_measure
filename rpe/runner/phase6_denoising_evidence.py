...
_WORKER_INPUTS: DenoisingEvidenceInputs | None = None
_WORKER_CONFIG: DenoisingEvidenceConfig | None = None
_WORKER_SYSTEMS: tuple[Phase3System, ...] = ()
_WORKER_IDENTITY_MODELS: Mapping[int, Mapping[str, object]] | None = None
_WORKER_IDENTITY_PREDICTIONS: Mapping[str, np.ndarray] | None = None
_WORKER_IDENTITY_LOSSES: Mapping[str, float] | None = None
_WORKER_IDENTITY_REFERENCE: Mapping[str, float] | None = None
_WORKER_IDENTITY_HALF_SPLIT: Mapping[str, float] | None = None
...
def _systems_from_catalog(config: DenoisingEvidenceConfig, project_root: Path) -> tuple[Phase3System, ...]:
    ...
    for system_id in config.system_ids:
        ...
        if system is None:
            raise DenoisingEvidenceError("catalog", f"missing system {system_id}")
        if system.task_line is not TaskLine.DENOISING:
            raise DenoisingEvidenceError("catalog", f"{system_id} is not a denoising system")
...
def _spectra_from_matrix(axis: np.ndarray, matrix: np.ndarray, record_ids: Sequence[str], well_ids: Sequence[str]) -> tuple[Spectrum1D, ...]:
    return tuple(
        Spectrum1D(
...
def _make_inputs(
    *,
    ...
    record_to_fold = {record_id: int(fold_by_well[well_id]) for record_id, well_id in zip(record_ids_tuple, well_ids_tuple, strict=True)}
...
def make_synthetic_denoising_evidence_inputs() -> DenoisingEvidenceInputs:
    ...
    for well_index in range(10):
        ...
        clean = background + np.sum((1.2 + np.arange(4, dtype="<f8"))[:, None] * target[:, None] * bases, axis=0)
...
def _identity_models(
...
) -> tuple[Mapping[int, Mapping[str, object]], Mapping[str, np.ndarray], Mapping[str, float], Mapping[str, float], Mapping[str, float], Mapping[str, float]]:
    ...
    model_digest_sha256 = hashlib.sha256(
        _jsonl_bytes(
            tuple(
                {
                    "fold": int(fold),
...
def _role_for_index(inputs: DenoisingEvidenceInputs, fold: int, index: int) -> str:
    if int(index) in set(int(value) for value in inputs.train_indices_by_fold[fold].tolist()):
        return "train"
    if int(index) in set(int(value) for value in inputs.validation_indices_by_fold[fold].tolist()):
        return "validation"
    return "test"
...
def _initialize_process(
    ...
    identity_predictions: Mapping[str, np.ndarray],
    identity_losses: Mapping[str, float],
    identity_reference: Mapping[str, float],
    identity_half_split: Mapping[str, float],
) -> None:
    global _WORKER_INPUTS, _WORKER_CONFIG, _WORKER_SYSTEMS
    global _WORKER_IDENTITY_MODELS, _WORKER_IDENTITY_PREDICTIONS, _WORKER_IDENTITY_LOSSES
    global _WORKER_IDENTITY_REFERENCE, _WORKER_IDENTITY_HALF_SPLIT
    _WORKER_INPUTS = inputs
    _WORKER_CONFIG = config
    _WORKER_SYSTEMS = tuple(systems)
    _WORKER_IDENTITY_MODELS = identity_models
    _WORKER_IDENTITY_PREDICTIONS = identity_predictions
    _WORKER_IDENTITY_LOSSES = identity_losses
    _WORKER_IDENTITY_REFERENCE = identity_reference
    _WORKER_IDENTITY_HALF_SPLIT = identity_half_split


def _evaluate_system_task(system_index: int):
    if (
        _WORKER_INPUTS is None
        or _WORKER_CONFIG is None
        or _WORKER_IDENTITY_MODELS is None
        or _WORKER_IDENTITY_PREDICTIONS is None
        or _WORKER_IDENTITY_LOSSES is None
        or _WORKER_IDENTITY_REFERENCE is None
        or _WORKER_IDENTITY_HALF_SPLIT is None
    ):
        raise DenoisingEvidenceError("worker", "process inputs are not initialized")
    system = _WORKER_SYSTEMS[int(system_index)]
    with threadpool_limits(limits=1, user_api="blas"):
        return _evaluate_system(
            inputs=_WORKER_INPUTS,
            config=_WORKER_CONFIG,
            system=system,
            identity_models=_WORKER_IDENTITY_MODELS,
            identity_predictions=_WORKER_IDENTITY_PREDICTIONS,
            identity_losses=_WORKER_IDENTITY_LOSSES,
            identity_reference_free=_WORKER_IDENTITY_REFERENCE,
            identity_half_split=_WORKER_IDENTITY_HALF_SPLIT,
        )
...
def build_phase6_denoising_evidence_from_inputs(
    inputs: DenoisingEvidenceInputs,
    systems: Sequence[Phase3System],
    output_root: Path,
    *,
    ...
    identity_models, identity_predictions, identity_losses, identity_reference, identity_half_split, identity_projection = _identity_models(inputs, config)
    ...
    system_indexes = tuple(range(len(ordered_systems)))
    if max_workers == 1:
        _initialize_process(
            inputs,
            config,
        ...
        results = [_evaluate_system_task(system_index) for system_index in system_indexes]
    else:
        with ProcessPoolExecutor(
            ...
            results = list(executor.map(_evaluate_system_task, system_indexes, chunksize=1))
    for system, result in zip(ordered_systems, results, strict=True):
        status, fit_rows, transform_rows, model_rows, downstream, aux_rows = result
    ...
    observed_counts = {
        "system_count": len(system_status_rows),
        "fit_receipt_count": len(fit_receipts),
        "transform_receipt_count": len(transform_receipts),
        "model_receipt_count": len(model_receipts),
        "downstream_well_row_count": len(downstream_rows),
        ...
        "system_status_row_count": len(system_status_rows),
    }
    enforce_formal_counts = tuple(system.system_id for system in ordered_systems) == tuple(config.system_ids)
    if enforce_formal_counts:
        for key, observed in observed_counts.items():
            if key in config.expected and int(config.expected[key]) != int(observed):
                raise DenoisingEvidenceError(key, f"expected {config.expected[key]} rows but observed {observed}")
    ...
    return DenoisingEvidenceSummary(
        path=path,
        run_id=run_id,
        status="complete",
        system_count=len(systems),
...
def build_phase6_denoising_evidence(
    output_root: Path,
    *,
    worker_count: int = 8,
    project_root: Path = ROOT,
    ...
    systems = _systems_from_catalog(config, project_root)
    inputs = _load_real_inputs(config, project_root)
    return build_phase6_denoising_evidence_from_inputs(
        inputs,
        systems,
        Path(output_root),
        config=config,
