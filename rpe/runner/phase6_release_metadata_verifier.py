"""Independent verifier for Phase 6 Step 9 release metadata artifacts."""
...
class ReleaseMetadataVerificationError(ValueError):
    """The Step 9 artifact does not match an independent rebuild."""
...
def _load_config(path: Path) -> _Config:
    ...
    if raw != _canonical(document):
        raise ReleaseMetadataVerificationError("config must be canonical JSON")
    if document.get("schema_version") != "phase6-release-metadata-v1":
        raise ReleaseMetadataVerificationError("unexpected config schema")
    return _Config(path=Path(path), sha256=_sha_bytes(raw), document=_freeze(document))


def _artifact_contract(config: _Config) -> tuple[str, ...]:
    ...
    if payloads != expected:
        raise ReleaseMetadataVerificationError("artifact_contract: payload inventory mismatch")
    return payloads
...
def _source_manifest_rows() -> Mapping[str, Mapping[str, object]]:
    ...
    for line in (ROOT / "data/raw/source_manifest.jsonl").read_bytes().splitlines():
        row = json.loads(line)
        rows[str(row["artifact_id"])] = MappingProxyType(row)
    return MappingProxyType(rows)


def _release_matrix_rows(*, mode: str) -> tuple[Mapping[str, object], ...]:
...
def _environment_payload(config: _Config, *, mode: str) -> bytes:
    value = {
...
def _croissant_payload(matrix_rows: Sequence[Mapping[str, object]]) -> bytes:
    distributions = []
...
def _step8_receipt(step8_path: Path | None) -> Mapping[str, object] | None:
    if step8_path is None:
        return None
    raw_path = Path(step8_path)
    path = (ROOT / raw_path).resolve() if not raw_path.is_absolute() else raw_path.resolve()
    if not path.is_dir():
        raise ReleaseMetadataVerificationError("step8 publication-core path must be an existing directory")
    required = ("manifest.json", "complete.json", "SHA256SUMS")
    for name in required:
        if not (path / name).is_file():
            raise ReleaseMetadataVerificationError(f"step8 publication-core receipt missing {name}")
    try:
        relative_path = path.relative_to(ROOT)
    except ValueError as error:
        raise ReleaseMetadataVerificationError("step8 publication-core path must be inside the repository root") from error
    ...
    if str(complete.get("status")) != "complete" or str(manifest.get("status")) != "complete":
        raise ReleaseMetadataVerificationError("step8 publication-core receipt must be complete")
    return MappingProxyType(
        {
...
def _run_id(config: _Config, *, mode: str, step8_receipt: Mapping[str, object] | None) -> str:
    payload = {
...
def _payloads(config: _Config, *, mode: str, step8_receipt: Mapping[str, object] | None, run_id: str) -> Mapping[str, bytes]:
    matrix_rows = _release_matrix_rows(mode=mode)
    matrix_fields = (
        "artifact_id",
        "title",
        "relative_path",
    ...
    croissant_json = _croissant_payload(matrix_rows)
    croissant_validation = _validate_croissant(croissant_json)
    if croissant_validation["status"] != "passed":
        raise ReleaseMetadataVerificationError(f"croissant validation failed: {croissant_validation['error']}")
    authority_bridge = {
...
def _inventory(path: Path, expected_payloads: tuple[str, ...]) -> None:
    found = {item.name for item in path.iterdir() if item.is_file()}
    expected = set(expected_payloads) | {"complete.json", "SHA256SUMS"}
    if found != expected:
        raise ReleaseMetadataVerificationError(
            f"artifact inventory mismatch: expected {sorted(expected)}, found {sorted(found)}"
        )
...
def _parse_sha256sums(path: Path, required_names: set[str]) -> Mapping[str, str]:
    ...
    for line in lines:
        ...
        if len(parts) != 2 or len(parts[0]) != 64 or parts[1] in listed:
            raise ReleaseMetadataVerificationError("invalid SHA256SUMS ledger")
    ...
    if set(listed) != actual_names:
        raise ReleaseMetadataVerificationError("SHA256SUMS inventory mismatch")
    if not required_names.issubset(listed):
        raise ReleaseMetadataVerificationError("SHA256SUMS required payload mismatch")
    for name, digest in listed.items():
        if _sha_file(path / name) != digest:
            raise ReleaseMetadataVerificationError(f"SHA256SUMS mismatch: {name}")
    return MappingProxyType(listed)


def _step8_from_authority_bridge(path: Path) -> Mapping[str, object] | None:
    bridge = json.loads((path / "authority_bridge.json").read_bytes())
    return bridge.get("step8_publication_core_receipt")


def verify_phase6_release_metadata(
    path: Path,
    *,
    ...
    expected_payloads = _artifact_contract(config)
    _inventory(path, expected_payloads)
    covered = set(expected_payloads) | {"complete.json"}
    _parse_sha256sums(path, covered)
    manifest = json.loads((path / "manifest.json").read_bytes())
    if str(manifest.get("status")) != "complete":
        raise ReleaseMetadataVerificationError("manifest status is not complete")
    mode = str(manifest.get("mode"))
    step8_receipt = _step8_from_authority_bridge(path)
    if mode == "formal":
        if step8_receipt is None:
            raise ReleaseMetadataVerificationError("formal artifact is missing Step8 receipt")
        verified_receipt = _step8_receipt(Path(str(step8_receipt["path"])))
        if dict(verified_receipt) != dict(step8_receipt):
            raise ReleaseMetadataVerificationError("step8 receipt mismatch")
    run_id = _run_id(config, mode=mode, step8_receipt=step8_receipt)
    if run_id != str(manifest["run_id"]):
        raise ReleaseMetadataVerificationError("run_id mismatch")
    rebuilt = _payloads(config, mode=mode, step8_receipt=step8_receipt, run_id=run_id)
    for name, payload in rebuilt.items():
        current = (path / name).read_bytes()
        if current != payload:
            raise ReleaseMetadataVerificationError(f"byte mismatch: {name}")
    return ReleaseMetadataSummary(
        path=path,
        run_id=str(manifest["run_id"]),
        status=str(manifest["status"]),
        release_row_count=int(manifest["release_row_count"]),
    )
