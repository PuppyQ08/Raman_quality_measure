...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    verify.add_argument("--worker-count", type=int, default=16)
    arguments = parser.parse_args(argv)
    try:
        if arguments.command == "build":
            summary = build_phase4_d5_eligibility(
                arguments.output_root, worker_count=arguments.worker_count
            )
        else:
            summary = verify_phase4_d5_eligibility(
                arguments.run_path,
                worker_count=arguments.worker_count,
            )
    except (OSError, ValueError, Phase4D5EligibilityError) as error:
        print(
            json.dumps(
                {
                    "error": str(error),
        ...
        json.dumps(
            {
                ...
                "split_occurrence_count": summary.split_occurrence_count,
